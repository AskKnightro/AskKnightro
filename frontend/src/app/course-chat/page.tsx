...
function CourseChatContent() {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!cRes.ok) {
          ...
          throw new Error(
            `Failed to load course (${cRes.status}): ${text || cRes.statusText}`
          );
        }
        ...
        if (!cancelled)
          setErr(e instanceof Error ? e.message : "Unable to load course.");
      } finally {
  ...
  const instructorName =
    teacher?.name ?? (course?.teacherId ? "Instructor" : "TBA");
          ...
          <form onSubmit={handleSendMessage} className={styles.inputForm}>
            <div className={styles.inputWrapper}>
              ...
              <button
                ...
                title={
                  !courseId
                    ? "Open from a course card so I know which class!"
                    : "Send"
                }
              >
                <svg
